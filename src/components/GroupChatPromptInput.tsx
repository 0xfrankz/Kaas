...
export function GroupChatPromptInput({ conversation }: Props) {
  ...
  const creator = useMessageCreator({
    onSuccess: (msg) => {
      // Update cache
      // Add user message to sub conversations' message list
      subConversations?.forEach((c) => {
        queryClient.setQueryData<Message[]>(
          [...LIST_MESSAGES_KEY, { conversationId: c.id }],
    ...
    onSettled: (msg, error) => {
      if (error) {
        ...
        // insert placeholder to all sub conversations to trigger generation
        subConversations?.forEach((c) => {
